...
<body> 
  <?php  
    ...
    mysqli_select_db($con,"user_info");//use user_info数据库； 
    $dbusername=null; 
    $dbpassword=null; 
    $result=mysqli_query($con,"select * from user_info where username ='{$username}';");//查出对应用户名的信息，isdelete表示在数据库已被删除的内容 
    while ($row=mysqli_fetch_array($result)) {//while循环将$result中的结果找出来 
      $dbusername=$row["username"]; 
      $dbpassword=$row["password"]; 
    } 
    if (is_null($dbusername)) {//用户名在数据库中不存在时跳回enter.html界面 
  ?> 
  ...
  <?php  
      ...
      if ($dbpassword!=$password){//当对应密码不对时跳回enter.html界面 
  ?> 
  <script type="text/javascript"> 
    alert("密码错误"); 
    window.location.href="enter.html"; 
